...
function Replace-StartMenuForAllUsers {
    ...
    Write-Host "> 正在为所有用户移除开始菜单中所有固定的应用..."
    ...
    if ($script:Params.ContainsKey("WhatIf")) {
        Write-Host "[WhatIf] 将使用模板 $startMenuTemplate 替换默认用户配置文件的开始菜单" -ForegroundColor Cyan
        return $true
    }
    ...
    if (-not (Test-Path $defaultStartMenuPath)) {
        try {
            ...
            Write-Warning "创建默认配置文件的开始菜单目录失败：$($_.Exception.Message)"
            return $false
        }
    }
...
function Replace-StartMenu {
    ...
    if ($script:Params.ContainsKey("WhatIf")) {
        Write-Host "[WhatIf] 将使用模板 $startMenuTemplate 替换用户 $userName 的开始菜单" -ForegroundColor Cyan
        return $true
    }
    ...
    try {
        if (Test-Path $startMenuBinFile) {
            ...
            Write-Host "找不到用户 $userName 的原始 start2.bin 文件，未为该用户创建备份" -ForegroundColor Yellow
        ...
        Write-Warning "为用户 ${userName} 替换开始菜单失败：$($_.Exception.Message)"
        return $false
    }
...
function Restore-StartMenu {
    ...
    Write-Host "正在从备份为用户 $targetUserName 还原开始菜单..."
...
function Restore-StartMenuForAllUsers {
    ...
    if (Test-Path $defaultStartMenuPath) {
        ...
        if (Test-Path -LiteralPath $defaultStartMenuBinFile) {
            if ($script:Params.ContainsKey("WhatIf")) {
                Write-Host "[WhatIf] 将移除默认用户配置文件的 start2.bin" -ForegroundColor Cyan
