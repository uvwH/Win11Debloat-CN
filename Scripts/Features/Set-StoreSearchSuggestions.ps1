...
function Set-StoreSearchSuggestionsDisabledForAllUsers {
    ...
    if ($processedProfiles -eq 0) {
        Write-Warning '无法禁用 Microsoft Store 搜索建议，因为无法解析任何目标用户配置文件。'
        return $false
    }
...
function Set-StoreSearchSuggestionsDisabled {
    ...
    if ($script:Params.ContainsKey("WhatIf")) {
        Write-Host "[WhatIf] 将通过限制对 ${StoreAppsDatabase} 的访问，为用户 $userName 禁用 Microsoft Store 搜索建议" -ForegroundColor Cyan
        return $true
    }
    ...
    try {
        ...
        if (-not (Test-Path -Path $StoreAppsDatabase)) {
            Write-Host "找不到用户 $userName 的 Store 应用数据库，现在先创建它，以防止 Windows 稍后重新创建..." -ForegroundColor Yellow
        ...
        Write-Warning "限制 Store 数据库 '$StoreAppsDatabase' 的 ACL 失败：$($_.Exception.Message)"
        return $false
    }
...
function Set-StoreSearchSuggestionsEnabledForAllUsers {
    ...
    if ($processedProfiles -eq 0) {
        Write-Warning '无法重新启用 Microsoft Store 搜索建议，因为无法解析任何目标用户配置文件。'
        return $false
    }
...
function Set-StoreSearchSuggestionsEnabled {
    ...
    if ($script:Params.ContainsKey("WhatIf")) {
        Write-Host "[WhatIf] 将通过恢复对 ${StoreAppsDatabase} 的访问，为用户 $userName 重新启用 Microsoft Store 搜索建议" -ForegroundColor Cyan
        return $true
    }
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Warning "撤销 Microsoft Store 搜索建议时，获取 Store 数据库 '$StoreAppsDatabase' 所有权失败。退出代码：$LASTEXITCODE"
        return $false
    }
    icacls "$StoreAppsDatabase" /grant *S-1-5-32-544:F /C | Out-Null
    if ($LASTEXITCODE -ne 0) {
        Write-Warning "撤销 Microsoft Store 搜索建议时，向 Store 数据库 '$StoreAppsDatabase' 授予管理员访问权限失败。退出代码：$LASTEXITCODE"
        return $false
    }
    ...
    try {
        ...
        Write-Warning "规范化 Store 数据库 '$StoreAppsDatabase' 的 ACL 失败：$($_.Exception.Message)"
    }
    ...
    try {
        ...
        Write-Warning "为用户 $userName 撤销 Microsoft Store 搜索建议时，移除 '$StoreAppsDatabase' 失败。$($_.Exception.Message)"
        return $false
    }
}
