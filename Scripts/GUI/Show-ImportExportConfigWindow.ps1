...
function Export-Configuration {
    ...
    Write-Host "正在将配置导出到 '$($saveDialog.FileName)'...（分类：$($categories -join ', ')）"
    ...
    if (Save-ToFile -Config $config -FilePath $saveDialog.FileName) {
        ...
        Write-Error "将配置导出到 '$($saveDialog.FileName)' 失败"
...
function Import-Configuration {
    ...
    if ($consistencyError) {
        Write-Error "无效的配置文件 '$($openDialog.FileName)'：$consistencyError"
    ...
    Write-Host "配置中可用的分类：$($availableCategories -join ', ')"
    ...
    if ($categories -contains 'System Tweaks' -and $config.Tweaks) {
        ...
        Write-Host "正在导入 $tweakCount 个调整项。"
