    ...
    $deploymentApplyBtn.Add_Click({
        ...
        if ($selectedApps.Count -gt 0) {
            ...
            if ([string]::IsNullOrWhiteSpace($scopeTarget)) {
                Write-Warning '由于所选的移除范围无效，已取消应用移除。'
                return
            }
        ...
        switch ($userSelectionCombo.SelectedIndex) {
            0 { Write-Host "已选择的用户模式：当前用户（$(Get-UserName)）" }
            1 {
                Write-Host "已选择的用户模式：$($otherUsernameTextBox.Text.Trim())"
                Add-Parameter User ($otherUsernameTextBox.Text.Trim())
            }
            2 {
                Write-Host "已选择的用户模式：默认用户配置文件（Sysprep）"
