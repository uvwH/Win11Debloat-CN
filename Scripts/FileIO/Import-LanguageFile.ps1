...
function Import-LanguageContent {
    ...
    # Optional per-language overrides for the app catalog (AppDetails.json):
    # maps AppId -> { FriendlyName, Description } and preset Name -> translated name.
    $appDetails = Import-JsonFile -filePath (Join-Path $folderPath 'AppDetails.json') -optionalFile
...
function Import-LanguageFile {
    ...
    if (-not $content -and $resolvedFolder -ne 'en-US') {
        Write-Warning "加载语言 '$resolvedFolder' 失败，正在回退到 en-US。"
        $content = Import-LanguageContent -LanguageFolder 'en-US' -LanguagesPath $LanguagesPath
    }

    if (-not $content) {
        Write-Error "无法加载 en-US 语言文件。GUI 缺少它将无法继续。"
        return $null
    }
...
function Get-Translation {
    ...
    if ($null -ne $FormatArgs -and $FormatArgs.Count -gt 0) {
        try {
            ...
            Write-Warning "翻译键 '$Key' 与其格式参数的占位符数量不匹配：$($_.Exception.Message)"
            return $resolved
        }
    }
...
function Test-LanguageKeyCoverage {
    ...
    if (-not $baseline -or -not $target) {
        Write-Error "无法加载用于覆盖率对比的语言内容（'$BaselineLanguageCode' 对比 '$resolvedLanguageCode'）。"
        return $null
    }
