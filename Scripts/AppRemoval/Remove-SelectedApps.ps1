...
function Get-AppRemovalMethod {
    ...
    if (-not $script:AppRemovalMethodCache) {
        ...
        try {
            ...
            Write-Warning "无法从 '$script:AppsListFilePath' 加载应用移除方式。未知的应用将默认使用 Appx。错误：$_"
        }
    }
...
function Request-EdgeForceRemove {
    ...
    elseif ($(Read-Host -Prompt "是否要强制卸载 Microsoft Edge？不推荐这么做！(y/n)") -eq 'y') {
        Write-Host ""
        return (Invoke-ForceRemoveEdge)
    }
...
function Set-RunOnceWingetTask {
    ...
    try {
        ...
        Write-Error "为 $($appId) 安排卸载任务失败：$_"
        return $false
    }
}
